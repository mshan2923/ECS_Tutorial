...
public partial class BoidSystemECS : SystemBase
{
    ...
    BoidControllerECSConponent controller;

    protected override void OnStartRunning()
    {
        Enabled = SystemAPI.HasSingleton<BoidControllerECSConponent>();
    }
    protected override void OnUpdate()
    {
        ...
        controller = SystemAPI.GetSingleton<BoidControllerECSConponent>();

        //if (controller)
        {
            EntityQuery boidQuery = GetEntityQuery(ComponentType.ReadOnly<BoidECS>(), ComponentType.ReadOnly<LocalToWorld>());
            NativeArray<float4x4> newBoidPositions = new NativeArray<float4x4>(boidQuery.CalculateEntityCount(), Allocator.TempJob);
            ...
            Entities.WithAll<BoidECS>().ForEach((Entity boid, ref LocalToWorld localToWorld)=>
            {
                ...
                Entities.WithAll<BoidECS>().ForEach((Entity otherBoid, ref LocalToWorld otherLocalToWorld) =>
                {
                    if (boid != otherBoid) 
                    {
                        ...
                        float distToOtherBoid = math.length(boidPosition - otherLocalToWorld.Position);//이게문제 1
                ...
                newBoidPositions[boidIndex] = float4x4.TRS(
                    localToWorld.Position + velocity * SystemAPI.Time.DeltaTime,
